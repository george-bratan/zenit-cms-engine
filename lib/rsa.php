...
class RSA {
	function encrypt($text, $key) {
		...
		foreach($in as $block) {
			if($block) {
				...
				$out .= $this->long_base_convert($cryptblock, 10, 62) . " ";
			}
		}
		return $out;
	}
	...
	function decrypt($code, $key) {
		...
		foreach($in as $block) {
			if($block) {
				$block = $this->long_base_convert($block, 62, 10);
	...
	function long_base_convert ($numstring, $frombase, $tobase) {
		...
		//$chars = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-+=!@#$%^*(){[}]|:,.?/`~•¤¶§ÇüéâäaaçeëeiîiÄAÉaAôöouuyÖÜcLYfáíóúnNao?¬11!«»—ßµ±÷;<>";
...
class RSA_keymaker {
	...
	function make_keys($bits = 1024, $u = false, $v = false) {
		...
		//print '<pre>';
		//print_r( Application::Profile() ); die();
	...
	function make_prime($bits) {
		...
		while(!$this->is_prime($num)) {
			...
			if (++$tries > 2001) return $this->make_prime($bits);
		}
		return $num;
	}
