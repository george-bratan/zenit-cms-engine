...
class CmsDocumentsManager {
      ...
      while (false !== ($file = readdir($handle))) {
        if($file != "." && $file != "..") {
          if(is_dir($current_path . $file))
            array_push($filesDir_dirs, $file);
          if(is_file($current_path . $file))
            array_push($filesDir_files, $file);
        }
      }
      closedir($handle);

      sort($filesDir_dirs); // sorting by names
      sort($filesDir_files); // sorting by names
      $filesDir = array_merge($filesDir_dirs, $filesDir_files);
  ...
  private function get_file_info($path='',$return=array()) {
    ...
    if(is_dir($this->doc_root . $path)) {
      ...
      $this->item['preview'] = $this->config['connector'] . '?mode=preview&path=' . rawurlencode($path);
