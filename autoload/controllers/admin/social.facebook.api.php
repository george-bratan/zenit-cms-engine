 ...
 */
class SocialFacebookApi extends BaseFacebook
{
  static function Init( $app, $secret )
  {
  	$api = new SocialFacebookApi(array(
  		'appId' => $app,
  		'secret' => $secret,
  		'cert' => Conf::Get('APP:ROOT') . '/autoload/other/fb_ca_chain_bundle.crt',
  	));

  	if (Session::Get('FACEBOOK.access.token')) {
  		$api->setAccessToken( Session::Get('FACEBOOK.access.token') );
  	}
  ...
  protected function setPersistentData($key, $value) {
    ...
    Session::Set("FACEBOOK.{$key}", $value);
  }
  ...
  protected function getPersistentData($key, $default = false) {
    ...
    return Session::Exists("FACEBOOK.{$key}") ? Session::Get("FACEBOOK.{$key}") : $default;
  }
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if (curl_errno($ch) == 60) { // CURLE_SSL_CACERT
      ...
      curl_setopt($ch, CURLOPT_CAINFO,
                  $this->cert ? $this->cert : dirname(__FILE__) . '/fb_ca_chain_bundle.crt');
      $result = curl_exec($ch);
    }
