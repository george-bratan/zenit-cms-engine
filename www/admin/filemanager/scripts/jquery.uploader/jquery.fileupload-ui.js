        ...
        _renderUploadTemplate: function (files) {
            ...
            */
            ...
            for (var i in files) {
	            ...
	            tmpl += '<tr class="template-upload '+ (file.error ? 'ui-state-error' : '') +'">'+
					        '<td class="preview"></td>';
					        //'<td class="name">'+ (file.name ? file.name : 'untitled') +'</td>'+
					        //'<td class="size">'+ file.sizef +'</td>';

				if (file.error) {
					//
					tmpl += '<td class="error">Error: ';

					if (file.error == 'maxFileSize')
						tmpl += 'File is too big';
					else if	(file.error == 'minFileSize')
						tmpl += 'File is too small';
					else if	(file.error == 'acceptFileTypes')
						tmpl += 'Filetype not allowed';
					else if	(file.error == 'maxNumberOfFiles')
						tmpl += 'Max number of files exceeded';
					else
						tmpl += file.error;
					...
					//
					tmpl += '<td class="progress"><p>File: '+ (file.name ? file.name : 'untitled') +'</p><p>Size: '+ file.sizef +'</p><div></div></td>'+
					        '<td class="start"><button>Start</button></td>';
				}

				tmpl += '<td class="cancel"><button>Cancel</button></td>';
				tmpl += '</tr>';
			}

			//alert(tmpl);

			return $(tmpl); //.appendTo('.fileupload-content table');
        },
        ...
        _renderDownloadTemplate: function (files) {
            ...
            */
            ...
            for (var i in files) {
				...
				tmpl += '<tr class="template-download '+(file.error ? 'ui-state-error' : '')+'">';
				if (file.error) {
					//
					tmpl += '<td></td>'+
				            //'<td class="name">'+file.name+'</td>'+
				            //'<td class="size">'+file.sizef+'</td>'+
				            '<td class="error"><p>'+file.name+'</p><p>'+file.sizef+'</p>Error: ';

					if (file.error == 1)
						tmpl += 'File exceeds upload_max_filesize (php.ini directive)';
					else if (file.error == 2)
						tmpl += 'File exceeds MAX_FILE_SIZE (HTML form directive)';
					else if (file.error == 3)
						tmpl += 'File was only partially uploaded';
					else if (file.error == 4)
						tmpl += 'No File was uploaded';
					else if (file.error == 5)
						tmpl += 'Missing a temporary folder';
					else if (file.error == 6)
						tmpl += 'Failed to write file to disk';
					else if (file.error == 7)
						tmpl += 'File upload stopped by extension';
						...
						tmpl += 'Uploaded bytes exceed file size';
					else if	(file.error == 'emptyResult')
						tmpl += 'Empty file upload result';
					else
						tmpl += file.error;
					...
					//
					tmpl += '<td class="preview">';

					if (file.thumbnail_url)
						tmpl += '<a href="'+file.url+'" target="_blank"><img class="thumb" src="'+file.thumbnail_url+'"></a>';
					else
						tmpl += '<a href="'+file.url+'" target="_blank"><img class="thumb" src="'+file.url+'"></a>';

					tmpl += '</td>'+
				            //'<td class="name">'+
				            //    '<a href="'+file.url+'" '+(file.thumbnail_url ? 'target="_blank"': '')+'>'+file.name+'</a>'+
				            //'</td>'+
				            //'<td class="size">'+file.sizef+'</td>'+
				            '<td class="progress">'+
				            	'<p>File: <a href="'+file.url+'" '+(file.thumbnail_url ? 'target="_blank"': '')+'>'+file.name+'</a></p><p>Size: '+file.sizef+'</p>' +
				            	'<div class="ui-progressbar"><div class="ui-progressbar-value">100%</div></div></td>';
				}

				tmpl += '<td class="delete" colspan=2>'+
