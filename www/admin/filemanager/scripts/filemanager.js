...
/*
if ($.urlParam('filebox') || $.urlParam('filefunc')) {
	if (window.parent) {
		if (!window.parent.fmloaded) {
			window.parent.fmloaded = true;
			return;
		}
	}
}
...
var setDimensions = function(){
	...
	$('#splitter').height(newH+2);
	$('#filetree, #fileinfo, .vsplitbar').height(newH);
}
...
var setUploader = function(path){
	...
	$('#uploader h1').text( displayPath(path) );
...
var selectItem = function(data){
	if(window.opener || $.urlParam('filebox') || $.urlParam('filefunc')){
		...
		if($.urlParam('filebox')){
			...
			window.parent.$('#'+$.urlParam('filebox')).focus().val(data['Path']).keyup().blur();
			parent.$.fancybox.close();
			return;
		}
		if($.urlParam('filefunc') && $.urlParam('param')){
			// use custom integration
			setTimeout("window.parent.$." + $.urlParam('filefunc') + "( '" + data['Path'] + "', '" + $.urlParam('param') + "' )", 100 );
			//parent.$.fancybox.close();
			return;
		}
		if($.urlParam('filefunc')){
			// use custom integration
			setTimeout("window.parent.$." + $.urlParam('filefunc') + "( '" + data['Path'] + "' )", 100 );
			//parent.$.fancybox.close();
			return;
		}
		...
		$.ajax({
			...
			success: function(result){
				if(result['Code'] == 0){
					...
					$('#uploader h1').text( displayPath(rootpath) );
					isDeleted = true;
...
var getFileInfo = function(file){
	...
	if(window.opener != null || $.urlParam('filebox') || $.urlParam('filefunc')) template += '<button id="select" name="select" type="button" value="Select">' + lg.select + '</button>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			properties += '<dt style="width:380px">&nbsp;</dt><dd style="width:380px"><a href="'+ parent.document.getElementById('UPLOAD').value + data['Path'] +'" target="_blank">' + parent.document.getElementById('UPLOAD').value + data['Path'] + '</a></dd>';
...
var populateFileTree = function(path, callback){
	var d = new Date(); // to prevent IE cache issues
	var url = fileConnector + '?path=' + encodeURIComponent(path) + '&mode=getfolder&onlydir=1&showThumbs=' + showThumbs + '&time=' + d.getMilliseconds();
	if ($.urlParam('type')) url += '&type=' + $.urlParam('type');
	$.getJSON(url, function(data) {
		var result = '';
		// Is there any error or user is unauthorized?
		...
		if(data) {
			...
			for(key in data) {
				...
				if (data[key]['File Type'] == 'dir') {
					...
					result += "<li class=\"file ext_" + data[key]['File Type'].toLowerCase() + "\" style=\"display:none\"><a href=\"#\" class=\"" + cap_classes + "\" rel=\"" + data[key]['Path'] + "\">" + data[key]['Filename'] + "</a></li>";
				}
			}
			result += "</ul>";
		} else {
...
$(function(){
	...
	$('#grid').click(function(){
		...
		$.cookie('ADMIN.DOCUMENTS.VIEW', 'GRID');
	});
	...
	$('#list').click(function(){
		...
		$.cookie('ADMIN.DOCUMENTS.VIEW', 'LIST');
	});

	// Provide initial values for upload form, status, etc.
	setUploader(fileRoot);

	$('#uploader').attr('action', fileConnector + '/upload');
	...
	$('body').fileupload({
		...
		if ($('.fileupload-content').length == 0) {
			$.prompt('<div class="fileupload-content">'+
					        '<div class="hint">You can also drag and drop a selection of files on the Documents area for upload.</div>' +
	...
	*/
	...
	if(window.opener == null && !$.urlParam('filebox') && !$.urlParam('filefunc')) $('#itemOptions a[href$="#select"]').remove();
    ...
    if ($.cookie('ADMIN.DOCUMENTS.VIEW') == 'LIST') {
		$('#list').click();
	}
});
