			...
			for (var j = 0; j < opts.markers.length; j++)
			{
				...
				if (marker.address)
				{
					...
					$geocoder.getLatLng(marker.address, function(gicon, marker)
					{
						...
						{
							if (marker.onlatlng) {
								marker.onlatlng( gpoint );
							}

							// Create marker
							gmarker = new GMarker(gpoint, {icon: gicon, draggable: marker.draggable});
							...
							if (marker.ondrop) {
								...
								marker.ondrop( gpoint.toUrlValue() );

								GEvent.addListener(gmarker, "dragend", function(){
									marker.ondrop( gmarker.getPoint().toUrlValue() );
								});
							}
						}
				...
				else
				{
					...
					gmarker = new GMarker(new GPoint(marker.longitude, marker.latitude), {icon: gicon, draggable: marker.draggable});
